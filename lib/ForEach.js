...
ForEach.textureInMaterial = function(material, handler) {
    // Metallic roughness
    ...
    if (defined(material.extensions)) {
        ...
        if (defined(pbrSpecularGlossiness)) {
            ...
            if (defined(pbrSpecularGlossiness.specularTexture)) {
                const textureInfo = pbrSpecularGlossiness.specularTexture;
                const value = handler(textureInfo.index, textureInfo);
                if (defined(value)) {
                    return value;
                }
            }
            if (defined(pbrSpecularGlossiness.glossinessTexture)) {
                const textureInfo = pbrSpecularGlossiness.glossinessTexture;
                const value = handler(textureInfo.index, textureInfo);
                if (defined(value)) {
                    return value;
                }
        ...
        if (defined(materialsCommon)) {
            if (defined(materialsCommon.diffuse) && defined(materialsCommon.diffuse.index)) {
                const textureInfo = materialsCommon.diffuse;
                const value = handler(textureInfo.index, textureInfo);
                if (defined(value)) {
                    return value;
                }
            }
            if (defined(materialsCommon.ambient) && defined(materialsCommon.ambient.index)) {
                const textureInfo = materialsCommon.ambient;
                const value = handler(textureInfo.index, textureInfo);
                if (defined(value)) {
                    return value;
                }
            }
            if (defined(materialsCommon.emission) && defined(materialsCommon.emission.index)) {
                const textureInfo = materialsCommon.emission;
                const value = handler(textureInfo.index, textureInfo);
                if (defined(value)) {
                    return value;
                }
            }
            if (defined(materialsCommon.specular) && defined(materialsCommon.specular.index)) {
                const textureInfo = materialsCommon.specular;
                const value = handler(textureInfo.index, textureInfo);
                if (defined(value)) {
                    return value;
                }
